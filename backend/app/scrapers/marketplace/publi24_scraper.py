"""Scraper Publi24.ro (Marketplace). curl_cffi AsyncSession + BeautifulSoup."""
import re
import urllib.parse
...
async def search_publi24(query: str, filters: dict = {}) -> list:
    query = (query or "").strip()
    if not query:
        return []
    filters = filters or {}
    url = f"{_BASE}/anunturi/q/{urllib.parse.quote(query)}/"
    ...
    try:
        async with AsyncSession() as session:
            ...
            if resp.status_code != 200:
                print(f"[publi24] HTTP {resp.status_code}")
                return []
            soup = BeautifulSoup(resp.text, "html.parser")
    except Exception as exc:
        print(f"[publi24] error: {exc}")
        return []
    ...
    for card in cards:
        try:
            ...
            loc_tag = card.find(class_=re.compile(r"location|oras|judet|locatie", re.I))
            ...
            results.append(make_result(
                ...
                source="publi24", platform_id=card.get("data-id"),
            ))
            if len(results) >= MAX_RESULTS:
                break
        except Exception as exc:
            print(f"[publi24] card parse error: {exc}")
            continue

    print(f"[publi24] {len(results)} rezultate pentru '{query}'")
