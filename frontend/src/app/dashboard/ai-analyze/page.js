...
import { Sparkles, Target, BarChart3, Package, Calendar, CheckCircle, XCircle, MinusCircle, AlertTriangle, ShoppingBag, Euro } from "lucide-react";

export default function AIAnalyzePage() {
  const [formData, setFormData] = useState({ product_name: "", category: "", price: "", source: "" });
  ...
  const getVerdictInfo = (verdict) => {
    if (!verdict) return { color: "#94a3b8", bg: "rgba(148,163,184,0.1)", icon: MinusCircle };
    ...
    <div style={{ maxWidth: "960px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.375rem" }}>
          ...
          <h1 style={{ fontSize: "1.5rem", fontWeight: 700, color: "white", margin: 0 }}>Analiza AI Produs</h1>
        </div>
        <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem", marginLeft: "3rem" }}>
          Analizeaza potentialul de profitabilitate al unui produs cu ajutorul AI
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
        {/* Form */}
        <div style={cardStyle}>
          <h2 style={{ fontSize: "1rem", fontWeight: 600, color: "white", marginBottom: "1.25rem" }}>Detalii produs</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {result ? (
            <>
                ...
                return (
                  <div style={{ ...cardStyle, border: `1px solid ${vi.color}33` }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                      <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "white", margin: 0 }}>Verdict</h3>
                    ...
                    <p style={{ color: "#cbd5e1", fontSize: "0.8125rem", lineHeight: 1.6, margin: 0 }}>
                      {result.explicatie_verdict || result.explicatie}
                    </p>
                  </div>
                );
              })()}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Scor Profitabilitate</span>
                  </div>
                ...
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Demand Score</span>
                  </div>
                  ...
                  {result.explicatie_demand && (
                    <p style={{ fontSize: "0.6875rem", color: "#64748b", marginTop: "0.375rem", lineHeight: 1.4 }}>
                      {result.explicatie_demand}
                    </p>
                  )}
                </div>
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Competitie</span>
                  </div>
                  <p style={{ fontSize: "1.125rem", fontWeight: 700, color: "white", margin: 0, textTransform: "capitalize" }}>{result.nivel_competitie}</p>
                ...
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Sezonalitate</span>
                  </div>
                  <p style={{ fontSize: "1.125rem", fontWeight: 700, color: "white", margin: 0 }}>{result.sezonalitate}</p>
              ...
              <div style={cardStyle}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.875rem" }}>
                  ...
                  <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "white", margin: 0 }}>Detalii financiare</h3>
                </div>
                <div>
                  {[
                    { label: "Pret achizitie", value: result.pret_achizitie_eur != null ? `${Number(result.pret_achizitie_eur).toFixed(2)} EUR` : "-", color: "#cbd5e1" },
                    { label: "Pret vanzare estimat", value: result.pret_vanzare_estimat != null ? `${Number(result.pret_vanzare_estimat).toFixed(2)} EUR` : "-", color: "white" },
                    { label: "Profit brut", value: result.profit_brut_eur != null ? `${Number(result.profit_brut_eur).toFixed(2)} EUR` : "-", color: "#cbd5e1" },
              ...
              {result.factori_risc?.length > 0 && (
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem" }}>
                    ...
                    <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "white", margin: 0 }}>Factori de risc</h3>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {result.factori_risc.map((risc, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", padding: "0.625rem", borderRadius: "0.5rem", backgroundColor: "rgba(251,146,60,0.08)", border: "1px solid rgba(251,146,60,0.2)" }}>
                        ...
                        <p style={{ fontSize: "0.8125rem", color: "#cbd5e1", margin: 0 }}>{risc}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {result.platforme_recomandate?.length > 0 && (
                <div style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem" }}>
                    ...
                    <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "white", margin: 0 }}>Platforme recomandate</h3>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                    {result.platforme_recomandate.map((p, i) => (
                      <span key={i} style={{ padding: "0.375rem 0.75rem", borderRadius: "0.375rem", fontSize: "0.8125rem", backgroundColor: "rgba(96,165,250,0.15)", color: "#93c5fd", fontWeight: 500 }}>
                        {p}
                      </span>
                    ))}
                  </div>
              ...
              {/* Tips */}
              <div style={cardStyle}>
                <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "white", marginBottom: "0.75rem" }}>Sfaturi AI</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {result.sfaturi?.map((sfat, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", padding: "0.625rem", borderRadius: "0.5rem", backgroundColor: "var(--bg-dark)" }}>
                      ...
                      <p style={{ fontSize: "0.8125rem", color: "#cbd5e1", margin: 0 }}>{sfat}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ ...cardStyle, padding: "3rem", textAlign: "center" }}>
              ...
              <p style={{ fontSize: "1rem", color: "white", marginBottom: "0.375rem" }}>Introdu datele produsului</p>
